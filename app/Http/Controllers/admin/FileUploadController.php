...
class FileUploadController extends Controller
{
    public function index(Request $request){
        ...
        return view('backend.upload_files.index', compact('all_uploads', 'search', 'sort_by'));
    }
    ...
    public function create(){
       ...
       return view('backend.upload_files.create');
    }


    public function show_uploader(Request $request){
        return view('admin.uploader.uploader-model');
    }
    public function upload(Request $request){
        ...
        if($request->hasFile('dsj_file')){
            $upload = new Upload;
            $extension = strtolower($request->file('dsj_file')->getClientOriginalExtension());

            if(isset($type[$extension])){
                $upload->file_original_name = null;
                $arr = explode('.', $request->file('dsj_file')->getClientOriginalName());
                ...
                $path = $request->file('dsj_file')->store('uploads/all', 'local');
                $size = $request->file('dsj_file')->getSize();
                $file_mime = $request->file('dsj_file')->getClientMimeType();
                ...
                if($type[$extension] == 'image'){
                    try {
                        $img = Image::make($request->file('dsj_file')->getRealPath())->encode();
                        $height = $img->height();
                        $width = $img->width();
                        
                        if($width > $height && $width > 1700){
                            $img->resize(1700, null, function ($constraint) {
                                $constraint->aspectRatio();
                            });
                        }elseif ($height > 1700) {
                            $img->resize(null, 1700, function ($constraint) {
                                $constraint->aspectRatio();
                            });
                        }else{
