...
<x-admin-layout page-title="{{ __('Blog') }}" page-description="">
    @section('breadcrumb')
        <x-breadcrumb :breadcrumbs="@[['title'=>__('Blog'), 'url' => '#' ]]" ></x-breadcrumb>
            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  ...
                  {{-- @can("Add User") --}}
                  <a href="{{ route('blog.add') }}" class="btn btn-primary btn-xs float-right">{{ __("Add") }}</a>
                  {{-- @endcan --}}
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    <table class="datatable_with_buttons table table-bordered table-hover">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>{{ __('Title')}}</th>
                                <th data-breakpoints="lg">{{ __('Category')}}</th>
                                <th data-breakpoints="lg">{{ __('Short Description')}}</th>
                            ...
                            <td>
                              ...
                              <div class="btn-group">
                                ...
                                <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                    {{-- @can("Edit") --}}
                                    <li>
                                      <a href="{{ route('blog.edit', $blog->id) }}" class="btn btn-link"><i class="dripicons-document-edit"></i> {{ __('Edit')}}</a>
                                    </li>
                                    {{-- @endcan
                                    @can("Delete") --}}
                                    <li class="divider"></li>
